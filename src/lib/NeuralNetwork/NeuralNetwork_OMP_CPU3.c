 ...
 * 
 *   This file implements the third of eight different OpenMP-optimised
...
void nn_train(neural_net* nn, size_t n_samples, double** inputs, double** expected, double learning_rate, size_t n_iterations) {
    ...
    for (size_t t = 0; t < n_threads; t++) {
        // Allocate arrays for the other layers except the first as we link that one
    ...
    double* delta_biases[n_samples][nn->n_weights];
    double* delta_weights[n_samples][nn->n_weights];
    for (size_t s = 0; s < n_samples; s++) {
        for (size_t l = 0; l < nn->n_weights; l++) {
            delta_biases[s][l] = malloc(sizeof(double) * nodes_per_layer[l + 1]);
            delta_weights[s][l] = malloc(sizeof(double) * nodes_per_layer[l] * nodes_per_layer[l + 1]);
        }
    }
    ...
    for (size_t i = 0; i < n_iterations; i++) {
        #pragma omp parallel
        {
            ...
            for (size_t s = 0; s < n_samples; s++) {
                ...
                // Set the inputs to the first layer output
            ...
            for (size_t s = 0; s < n_samples; s++) {
                for (size_t l = 0; l < nn->n_weights; l++) {
                    ...
                    double* delta_bias = delta_biases[s][l];
                    double* weight = weights[l];
                    double* delta_weight = delta_weights[s][l];
    ...
    for(size_t s = 0; s < n_samples; s++) {
        for(size_t l = 0; l < n_layers - 1; l++) {
            free(delta_biases[s][l]);
            free(delta_weights[s][l]);
        }
    }
...
void print_opt_args() {
    printf(" - Variation               : OpenMP CPU 3 (Forward & Backward, reduction)\n");
